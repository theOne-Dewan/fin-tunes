...
function Center() {
  ...
  return (
    <div className="bg-black flex-grow col-span-full relative h-screen overflow-y-scroll scrollbar-hide">
      <header className="absolute top-5 right-8 flex items-center space-x-4">
        <div className="flex items-center bg-black space-x-3 text-white opacity-90 rounded-full p-1 pr-2 hover:opacity-80 cursor-pointer">
          <img className="rounded-full w-10 h-10" src={session?.user.image}/>
          <h2>{session?.user.name}</h2>
          <ChevronDownIcon className="h-5 w-5" />
        </div>
        <div onClick={signOut} className="bg-black space-x-3 text-white opacity-90 rounded-full p-3.5 pr-2.5 hover:opacity-80 cursor-pointer">
